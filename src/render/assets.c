...
mesh_handle assets_load_mesh(assets_t *a, arena_t *scratch, const char *path) {
    ...
    ASSERT_RET_MSG(a->mesh_count < ASSETS_MAX_MESHES, ASSET_INVALID,
                   "assets: mesh capacity exceeded");
    u32 h = a->mesh_count++;
    a->meshes[h] = mesh_load_gltf(scratch, path);
    a->mesh_paths[h] = arena_strdup(a->permanent, path);
    LOG_DEBUG("assets: loaded mesh [%u] %s", h, path);
    return h;
}
...
texture_handle assets_load_texture(assets_t *a, arena_t *scratch, const char *path, b32 flip) {
    ...
    ASSERT_RET_MSG(a->texture_count < ASSETS_MAX_TEXTURES, ASSET_INVALID,
                   "assets: texture capacity exceeded");
    u32 h = a->texture_count++;
    a->textures[h] = texture_load(scratch, path, flip);
    a->texture_paths[h] = arena_strdup(a->permanent, path);
    LOG_DEBUG("assets: loaded texture [%u] %s", h, path);
    return h;
}
